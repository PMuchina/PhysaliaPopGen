...
## Part 3: Introduction to demographic modellingNovember 27, 2024 
...
How many biallelic SNPs are present in the VCF file? 
OPTIONAL: if you have estimated the variant density on the first scaffold of the vcf (see first OPTIONAL section above), does this value change assuming biallelic SNP only? Why?
...
1/ Please again, review the code carefully to understand the rationale, but DO NOT EXECUTE it on the Physalia cluster today! Please only execute the code from the section corresponding to the "smc++ plot (...)", see below (from line starting with "conda deactivate"...). 
...
The results are available on the following directory (i.e. the same than before) with two outputs files: "physalia_smcpp_plot_missingcutoff500.csv" and "physalia_smcpp_plot_missingcutoff500.pdf":
cd ~/Day3_demography/data-datepalm/Res_SMCpp/
...
Similarly to before, some warnings are expected to be shown on the screen, no worries. At the end, this R script is expected to output a pdf containing the results.
